...
class ChongQingTencentSpider(Spider.Spider):
	"""腾讯-大渝网 Spider"""
	def __init__(self):
	...
	def CatchArticles(self):
		...
		recDate = re.compile('/([\d]+?)/\d+?.htm', re.DOTALL)
		recContent = re.compile('<!--link-->((?:[^<]|<(?!\!--/link-->))+?)<!--/link-->', re.DOTALL)
		...
		for x in recAbstract.findall(html):
			...
			for y in recArticle.findall(html):
				content = y
				for c in recContent.findall(content):
					logging.debug('复合文章源{0}文字内容为{1}。'.format(article['url'], c))
					content = c
